...
const facets = computed(() => {
  ...
  // Trier les plateformes: celles qui commencent par un chiffre à la fin
  const sortedPlatforms = Array.from(platforms).sort((a, b) => {
    const aStartsWithDigit = /^\d/.test(a)
    const bStartsWithDigit = /^\d/.test(b)

    // Si a commence par un chiffre et pas b, a va après b
    if (aStartsWithDigit && !bStartsWithDigit) return 1
    // Si b commence par un chiffre et pas a, b va après a
    if (!aStartsWithDigit && bStartsWithDigit) return -1
